...
(** Yet another Imp variant, this time with types!
    ...
    _Soundness_ for _Typed Imp_ (TImp) is a good example of a realistic
...
(** Instead of writing tedious manual instances for simple
    inductive types like [ty], [nat], etc., we can use QuickChick's
...
(** To encode typing environments (and, later on, states), we
    will need maps from identifiers to values.

    Why not just use the _Logical Foundations_ version? Because it
    uses a _function-based_ representation.

    For testing, we need to
      - access the domain of the map,
      - test the map for equality, and
      - iterate through the map.
    These are awkward for the functional representation. *)

(**
    Instead we're going to use a list-based representation.
...
(** We can now decide whether [bound_to m x a] holds for any
    [m], [x] and [a] (the proof details are elided): *)
