 ...
 * slides. Pressing escape brings the page back to normal. Page Up and
 * Page Down jump to the top of the previous / next slide in either mode. */
...
/* Page Up / Page Down: jump to the top of the previous / next slide
 * boundary relative to the current scroll position, entering slide
 * mode first so that only one slide is on screen. */
function slidePage(direction) {
    /* jsCoq wraps the document in a scrolling div#page; plain pages
     * scroll the document itself. Measure against whichever scrolls. */
    var box = document.getElementById('page') || document.scrollingElement;
    var top = box.getBoundingClientRect().top;
    var target = null;
    $('.slide').each(function (i, elt) {
        var y = elt.getBoundingClientRect().top - top;
        if (direction == 'down') {
            if (y > 2 && target == null) target = elt;
        } else {
            if (y < -2) target = elt;
        }
    });
    if (target == null) return;
    if (!slideMode) slideActivate();
...
/* Page Up / Page Down always page between slides, also while an
 * editor has focus. Registered in the capture phase so CodeMirror never
 * sees the key; the editor is blurred so the arrows work afterwards. */
document.addEventListener('keydown', function (event) {
    if (event.keyCode != KEYS.pgup && event.keyCode != KEYS.pgdn) return;
    event.preventDefault();
    event.stopPropagation();
    if (document.activeElement && document.activeElement.blur)
        document.activeElement.blur();
    slidePage(event.keyCode == KEYS.pgdn ? 'down' : 'up');
}, true);

$(document).keydown(function (event) {
    /* Leave the arrow keys alone while an editor has focus (jsCoq's
     * CodeMirror panes, inputs); otherwise cursor movement in a proof
     * flips slides. */
    if ($(event.target).closest('.CodeMirror, input, textarea, [contenteditable]').length)
        return true;
    if (slideMode) {
